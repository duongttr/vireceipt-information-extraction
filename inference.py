...
import torch
import os
from utils import draw_output, normalize_box, unnormalize_box, create_json
import numpy as np
...
class LayoutLMv3:
    ...
    def predict(self, input_image, output_path=None):
        ...
        # final_results = {'text': [], 'label': []}
        # for box, text, label in zip(true_boxes, true_texts, true_predictions):
        #     final_results['text'].append(text)
        #     final_results['label'].append(label)
