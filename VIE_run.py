...
@st.cache_resource
def init_models():
    model_path = os.path.join("src", "model", "models", "segmentation_model.h5")
...
for option in options:
    if option == "Blur":
        blur = st.sidebar.text_input(
            "Amount of blur",
            "0",
            help="The standard deviation value. The higher the standard deviation, the more blurred the image is",
        )
    if option == "Contrast":
...
with st.spinner(random.choice(waiting)):
    if uploaded_file:
            ...
            if process_button:
                ...
                for option in options:
                    if option == "Warp":
                        ...
                        bin_image = fn[option](bin_image, mode, int(block_size), int(constant))
                    elif option == "Canny":
                        bin_image = fn[option](bin_image)
            st.image(bin_image)
        ...
        if result:
            st.download_button(
                label="Download data as JSON", data=result, file_name="extracted_data.json", mime="application/json"
            )
